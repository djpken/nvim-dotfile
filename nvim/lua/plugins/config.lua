            ...
            edit = {
                ...
                function()
                    ...
                    if node.name ~= ".." and node.fs_stat.type == "file" then
                        ...
                        if table.find(extensions_opened_externally,
                                      node.extension) then
                            api.node.run.system()
                            return
                        end
...
config.bufferline = {
        ...
        options = {
            ...
            diagnostics_indicator = function(_, _, diagnostics_dict, _)
                ...
                for e, n in pairs(diagnostics_dict) do
                    local sym = e == "error" and symbols.Error or
                                    (e == "warning" and symbols.Warn or
                                        symbols.Info)
                    s = s .. n .. sym
                end
                return s
    ...
    config = function(_, opts)
        vim.api.nvim_create_user_command("BufferLineClose",
                                         function(buffer_line_opts)
            local bufnr = 1 * buffer_line_opts.args
            local buf_is_modified = vim.api.nvim_get_option_value("modified",
                                                                  {buf = bufnr})
            ...
            if buf_is_modified then
                local option = vim.fn.confirm(
                                   "File is not saved. Close anyway?",
                                   "&Yes\n&No", 2)
                if option == 1 then vim.cmd(command) end
            else
                vim.cmd(command)
            end
        end, {nargs = 1})
    ...
    opts = {
        ...
        config = {
            ...
            header = {
                ...
                "╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝  ╚═══╝  ╚═╝╚═╝     ╚═╝",
                " ",
                string.format("Nvim: v%s   KkNvim: v%s  Colorscheme: %s",
                              require("core.utils").version, "0.0.0",
            ...
            center = {
...

                {
                    ...
                    action = string.format("edit %s/lua/custom/init.lua",
                                           config_root),
                    key = "e"
                },
                {
            ...
            footer = function()
                ...
                return {
                    "⚡ KkNvim loaded " .. stats.loaded .. "/" .. stats.count ..
                        " plugins in " .. ms .. "ms",
                    " ",
...
config["flutter-tools"] = {
    ...
    opts = {
        ...
        lsp = {on_attach = function(_, bufnr) Kk.lsp.keyAttach(bufnr) end}
    }
}
...
config["nvim-transparent"] = {
    ...
    config = function(_, opts)
        local autogroup = vim.api.nvim_create_augroup("transparent",
                                                      {clear = true})
        vim.api.nvim_create_autocmd("ColorScheme", {
            ...
            callback = function()
                ...
                vim.api.nvim_command("highlight default KkNormal guifg=" ..
                                         foreground .. " guibg=" .. background)
...
config["rust-tools"] = {
    ...
    main = "rust-tools",
    event = "User KkLoad",
    opts = {server = {on_attach = function(_, bufnr) Kk.lsp.keyAttach(bufnr) end}}
}
config.surround = {
...
config["zen-mode"] = {
    ...
    opts = {
        ...
        on_open = function() vim.opt.cmdheight = 1 end,
        on_close = function() vim.opt.cmdheight = 2 end
    },
    config = function(_, opts)
...
config.mason = {
    ...
    config = function()
        ...
        local function install(package)
            ...
            if s and not p:is_installed() then p:install() end
        end

        for _, package in pairs(Kk.lsp.ensure_installed) do
            if type(package) == "table" then
                for _, p in pairs(package) do install(p) end
            else
                install(package)
            end
        end
...
config["smartim"] = {
    "ybian/smartim",
    event = "User KkLoad",
    config = function() vim.g.smartim_default = "com.apple.keylayout.ABC" end
}
config["vim-go"] = {
    "fatih/vim-go",
    event = "User KkLoad",
    keys = {{"gr", "<Plug>(go-referrers)"}},
    config = function() vim.g.go_def_mapping_enabled = 0 end
}
